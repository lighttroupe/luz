class GuiDefault < GuiBox
	...
	def create_default_gui
		...
		self << (@text = BitmapFont.new.set(:string => 'Luz 2.0 has text support!!', :offset_x => -0.3, :offset_y => 0.5 - 0.03, :scale_x => 0.02, :scale_y => 0.04))
	...
	def build_editor_for(user_object, options)
		@text.set_string("Clicked on '#{user_object.title}'")
	end
end
