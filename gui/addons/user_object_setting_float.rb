class UserObjectSettingRenderer < GuiBox
	def create_user_object_setting_name_label
		@name_label ||= GuiLabel.new.set(:string => @setting.name.gsub('_',' '), :color => [1.0,1.0,1.0,1.0], :scale_x => 1.0, :scale_y => 0.35, :offset_x => -0.02, :offset_y => 0.40)
	end
end
	...
	def create!
		...
		row = GuiHBox.new	#.set(:scale_y => 0.5, :offset_y => 0.23)
			...
			unless @setting.options[:simple]
				...
				row << (@animation_every_text=GuiLabel.new.set(:string => 'every', :offset_x => 0.025, :scale_x => 0.1, :scale_y => 0.5, :float => :left, :opacity => 0.4))
		...
		unless @setting.options[:simple]
			...
			row << (@activation_when_text=GuiLabel.new.set(:string => 'when', :offset_x => 0.025, :scale_x => 0.1, :scale_y => 0.5, :float => :left, :opacity => 0.4))
