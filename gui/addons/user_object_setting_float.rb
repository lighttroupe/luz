	...
	def create!
		...
		row = GuiHBox.new	#.set(:scale_y => 0.5, :offset_y => 0.23)
			...
			unless @setting.options[:simple]
				...
				row << (@animation_curve_widget=GuiCurve.new(@setting, :animation_curve).set(:scale_x => 0.13, :scale_y => 0.8, :float => :left, :opacity => 0.0, :hidden => true))
				row << (@animation_max_widget=GuiFloat.new(@setting, :animation_max, @min, @max).set(:scale_x => 0.15, :float => :left, :opacity => 0.0, :hidden => true))
				...
				row << (@animation_repeat_number_widget=GuiFloat.new(@setting, :animation_repeat_number, 0.25, 128.0).set(:step_amount => 0.25, :scale_x => 0.2, :float => :left, :opacity => 0.0, :hidden => true))
				row << (@animation_repeat_unit_widget=GuiSelect.new(@setting, :animation_repeat_unit, UserObjectSettingFloat::TIME_UNIT_OPTIONS).set(:width => 5, :text_align => :left, :scale_x => 0.15, :float => :left, :opacity => 0.0, :hidden => true))
		...
		unless @setting.options[:simple]
			...
			row << (@activation_curve_widget=GuiCurveIncreasing.new(@setting, :activation_curve).set(:scale_x => 0.13, :scale_y => 0.8, :float => :left, :opacity => 0.0, :hidden => true))
			row << (@activation_direction_widget=GuiSelect.new(@setting, :activation_direction, UserObjectSettingFloat::ACTIVATION_DIRECTION_OPTIONS).set(:width => 4, :text_align => :center, :scale_x => 0.1, :float => :left, :opacity => 0.0, :hidden => true))
			row << (@activation_value_widget=GuiFloat.new(@setting, :activation_value, @min, @max).set(:scale_x => 0.15, :float => :left, :opacity => 0.0, :hidden => true))
			...
			row << (@activation_variable_widget=GuiVariable.new(@setting, :activation_variable).set(:no_value_text => 'variable', :scale_x => 0.26, :float => :left, :opacity => 0.0, :hidden => true))
