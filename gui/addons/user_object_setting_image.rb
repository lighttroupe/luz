class UserObjectSettingImage
	def gui_build_editor
		...
		box << @open_button = GuiButton.new.set(:scale_x => 0.1, :float => :left, :scale_y => 0.5, :offset_y => 0.0, :background_image => $engine.load_image('images/buttons/image-select.png'))
		...
		box << @clear_button = GuiButton.new.set(:scale_x => 0.1, :float => :left, :scale_y => 0.5, :offset_x => 0.02, :offset_y => 0.0, :background_image => $engine.load_image('images/buttons/clear.png'))
